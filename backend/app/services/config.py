...
Consumers import from here for backward compatibility:
    from app.services.config import REQUIRED_FIELDS, PROCEED_PHRASES, ...
"""
...
def __getattr__(name: str):
    ...
    if name in _DYNAMIC_MAP:
        from app.services.dynamic_config import LEGACY_RULES as _lr
        if _lr:
            return _LEGACY[name]
        from app.services.dynamic_config import get_vocabulary
        return _DYNAMIC_MAP[name](get_vocabulary())
    raise AttributeError(f"module 'app.services.config' has no attribute {name!r}")
