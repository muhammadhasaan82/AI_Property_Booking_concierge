...
`backend/app/agents/` contains the three-node ADK pipeline in `adk_agents.py`, the `UnderstandingFrame` contract under `schemas/`, the state helpers under `state/`, and the Python tool layer under `tools/`.

`backend/app/components/` holds the retrieval stack: hybrid vector and BM25 search, FAQ RAG over policy PDFs in `faq_enhanced.py`, and the NLP engine in `nlp_engine.py`.

`backend/app/config/` is the control plane. `agent_config.yaml` drives thresholds and fallbacks, `tool_registry.yaml` declares tools, `agent_policy.yaml` encodes deterministic routing, `booking_schema.yaml` defines booking fields, and `response_policies.yaml` shapes the voice node.

`backend/app/observability/` contains DPO telemetry, DB logging, and tracing.

`backend/app/prompts/` stores the externalized Markdown prompts for understanding, triage, and voice.

`backend/app/route/` contains FastAPI handlers for chat, admin, health, properties, booking, FAQ, mobile, and the Stripe webhook.

`backend/app/security/` holds `anomaly.py`, `guardrails.py`, and the deterministic `policy_router.py`.

`backend/app/services/` wires the ADK runner, Redis session service, pre-router, Mem0 memory engine, TOON codec, booking, Supabase/Postgres client, and dynamic config loaders.

`backend/rust_gateway/` contains the Axum service, CAG intercept, rate limiter, TOON codec, and tool implementations.

`backend/scripts/` contains `export_dpo_dataset.py`, `finetune_openai.py`, `generate_cag_policies.py`, `rebuild_rag_index.py`, and `initialize_faq.py`.

`backend/evaluation/` holds the offline evaluation suite referenced by `.github/workflows/ci.yml`.

`frontend/` contains `chainlit_app.py` and the custom branding assets under `frontend/public/`.

`docs/` covers `architecture.md`, `api-reference.md`, `deployment.md`, and `finetuning_playbook.md`.
